...
function common(res, data, type) {
  const head_item = data[0]
  ...
  } else if (type === 'judge' || type === 'blank') {
    if (title_index === -1 || result_index === -1) {
      res.json({
        code: -1,
        msg: '文件读取的格式没有按照要求来 ',
        data: null,
...
router.post('/readFile', upload.single('file'), function (req, res, next) {
  ...
  try {
    ...
    console.log(JSON.stringify(obj[1]['data']))
    const read_data1 = obj[0]['data']
    const read_data2 = obj[1]['data']
    const read_data3 = obj[2]['data']
    const read_data4 = obj[3]['data']
    let sign_data = {}
    let mult_data = {}
    let judge_data = {}
    let blank_data = {}

    sign_data = common(res, read_data1, 'choose')
    mult_data = common(res, read_data2, 'choose')
    judge_data = common(res, read_data3, 'judge')
    blank_data = common(res, read_data4, 'blank')
    res.json({
      code: 1,
      msg: '文件读取成功 ',
      data: {
